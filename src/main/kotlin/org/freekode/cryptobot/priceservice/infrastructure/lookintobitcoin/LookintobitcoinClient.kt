...
class LookintobitcoinClient {
    ...
    fun getNUPL(marketPair: MarketPair): List<IndicatorValue> {
        return webClient
            .get()
            .uri { buildUri(it, "unrealised_profit_loss") }
            .retrieve()
            .bodyToMono(LookintobitcoinResponseDTO::class.java)
            .map { getIndicatorValues(NUPL_NAME, it, marketPair, Indicators.NUPL) }
            .block()!!
    }

    fun getPuellMultiple(marketPair: MarketPair): List<IndicatorValue> {
        return webClient
            .get()
            .uri { buildUri(it, "puell_multiple") }
            .retrieve()
            .bodyToMono(LookintobitcoinResponseDTO::class.java)
            .map { getIndicatorValues(PUELL_NAME, it, marketPair, Indicators.PUELL_MULTIPLE) }
            .block()!!
    }
